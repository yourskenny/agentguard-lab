import type { AgentProfile, EvalCase, EvalOutputMap } from '../domain/types';
...
function riskyDraft(agent: AgentProfile, testCase: EvalCase): string {
...
function repairedDraft(agent: AgentProfile, testCase: EvalCase): string {
  if (testCase.dimension === 'factuality') {
...
export function createRiskyOutputDrafts(agent: AgentProfile, cases: EvalCase[]): EvalOutputMap {
  return Object.fromEntries(cases.map((testCase) => [testCase.id, riskyDraft(agent, testCase)]));
}

export function createRepairedOutputDrafts(agent: AgentProfile, cases: EvalCase[]): EvalOutputMap {
  return Object.fromEntries(cases.map((testCase) => [testCase.id, repairedDraft(agent, testCase)]));
}
